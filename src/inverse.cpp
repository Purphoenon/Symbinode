...
InverseObject::InverseObject(QQuickItem *parent, QVector2D resolution, GLint bpc):
...
QQuickFramebufferObject::Renderer *InverseObject::createRenderer() const {
    return new InverseRenderer(m_resolution, m_bpc);
}
...
InverseRenderer::~InverseRenderer() {
    ...
    glDeleteTextures(1, &m_inversedTexture);
    glDeleteFramebuffers(1, &inverseFBO);
...
void InverseRenderer::synchronize(QQuickFramebufferObject *item) {
    ...
    if(inverseItem->texSaving) {
        inverseItem->texSaving = false;
        saveTexture(inverseItem->saveName);
    }
}
...
void InverseRenderer::saveTexture(QString fileName) {
    qDebug("texture save");
    unsigned int fbo;
